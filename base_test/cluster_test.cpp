...
TEST(Cluster, cant_create_cluster_with_too_small_tacts)
{
	ASSERT_ANY_THROW(Cluster cl(10, MinTacts - 1, 40, 2, 0.4));
}
TEST(Cluster, cant_create_cluster_too_large_tacts)
{
	ASSERT_ANY_THROW(Cluster cl(10, MaxTacts + 1, 40, 2, 0.4));
}
TEST(Cluster, cant_create_cluster_with_too_small_nodes)
{
	ASSERT_ANY_THROW(Cluster cl(10, 20, MinN-1, 2, 0.4));
}
TEST(Cluster, cant_create_cluster_with_too_large_nodes)
{
	ASSERT_ANY_THROW(Cluster cl(10, 20, MaxN + 1, 2, 0.4));
}
TEST(Cluster, cant_create_cluster_with_too_small_parametr_k)
{
