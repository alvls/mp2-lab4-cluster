	...
	: length(other.length), first(nullptr), last(nullptr)
{
	...
	TLinkedList::Node* cur, * sCur = other.first;
	cur = first = new TLinkedList::Node { other.first };

	while (sCur->next)
	{
		cur->next = new TLinkedList::Node{ sCur->value };
		cur = cur->next;
		sCur = sCur->next;
	}
	last = cur;
...
template<typename T>
typename TLinkedList<T>::Node* TLinkedList<T>::node_at(size_t pos)
{
...
template<typename T>
void swap(TLinkedList<T>& lhs, TLinkedList<T>& rhs)
{
